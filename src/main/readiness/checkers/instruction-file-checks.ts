...
export function resolveInstructionFilePath(projectPath: string, opts: InstructionFileOpts): string | null {
  const candidates = [joinStoredProjectPath(projectPath, opts.fileName)];

  if (opts.fallbackDirectory) {
    candidates.push(joinStoredProjectPath(projectPath, path.join(opts.fallbackDirectory, opts.fileName)));
  }
